...
const height = 600;
const width = 800;
const nodeYTranslation = 0

const fixNodes = {env: {x: width/2, y: height/4, rx: 50, ry: 30}, seed: {x: width/2, y: height/2.8, rx: 10, ry: 10}}
    ...
    .join(
      (enter) =>
        enter
          ...
          .attr("opacity", d => { // Do we want dying or dividing cells to have a measure of self-efficacy 
            if (d.class === "non-Seed") {
              // console.log(d.self_efficacy)
              return d.self_efficacy + .5
            }
            else {
              return 1
            }
  ...
  function ticked() {
    ...
    links
      ...
      .attr("y1", (d: any) => {
        ...
        else { return height/2 + nodeYTranslation }
...
export function setupNetwork(element: string, playButton: string, max_timestamps: number) {
  ...
  select(playButton).on("click", function () {
    ...
    if (isPlaying) {
      ...
      animationInterval = setInterval(function () {
        ...
        // selectAll(".numCellsTimeline").remove()
        ...
        animateNumCells("#numCells", index)
        animateMessages("#messages", index)
        animateNailed("#nailed", index)
        animateSeedCellSuffering("#seedCellSuffering", index)
        // animateStemnesses("#stemness", index)
        // animateStresses("#stresses", index)
        // runSimulation("#prerender-canvas", height, width, staticGraph as unknown as Graph, index);  
        // select("#timeslider").attr("value", index);
      }, 1000);
    } else {
  ...
  // let zoom = d3.zoom<any,any>() 
  //   .on('zoom', handleZoom);

  // function handleZoom(e: any) {
  // d3.select('#prerender-canvas')
